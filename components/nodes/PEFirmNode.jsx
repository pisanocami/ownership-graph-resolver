...
export default function PEFirmNode({ data, selected }) {
  ...
  return (
    <div
      className={`flow-node pe-firm ${isFocal ? 'focal' : ''} ${selected ? 'selected' : ''}`}
      onClick={() => onSelect?.(node.company)}
    >
      <Handle type="target" position={Position.Top} style={handleStyle} />
      <div className="flow-node-name">
        <span style={{ marginRight: 4 }}>💼</span>{node.company}
      </div>
      {node.stake_pct && (
        <div className="flow-node-detail" style={{ fontSize: 10, marginTop: 4, color: 'var(--text-muted)' }}>
          {node.stake_pct}% stake
        </div>
      )}
